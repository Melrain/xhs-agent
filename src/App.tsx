...
export default function App() {
  ...
  const scopedComments = useMemo(() => {
    ...
    return comments.filter((comment) => {
      if (selectedId && comment.noteId !== selectedId) {
        return false;
      }
      if (!text) {
        return true;
  ...
  async function exportCurrent() {
    ...
    try {
      ...
      if (path) {
        setExportHint(`已导出 ${scopedComments.length} 条到 ${path}`);
      }
    } catch (error) {
      setInvokeError(asMessage(error));
    } finally {
  ...
  async function removeAccount() {
    ...
    if (!window.confirm('移出后这个号的 session 会删掉，笔记还在。确定？')) {
      return;
    }
    setBusy(true);
    setInvokeError(null);
  ...
  async function applyUpdate() {
    ...
    try {
      setUpdateStatus({ kind: 'downloading', version: update.version, percent: 0 });
      await installAppUpdate(update, (percent) => {
        setUpdateStatus({ kind: 'downloading', version: update.version, percent });
      });
      setUpdateStatus({ kind: 'installing', version: update.version });
    } catch (error) {
      setUpdateStatus({ kind: 'error', message: asMessage(error) });
    }
  }
  ...
  return (
    <div className="app">
      ...
      {updateStatus.kind !== 'idle' && updateStatus.kind !== 'checking' ? (
        <div className={updateStatus.kind === 'error' ? 'update-bar update-error' : 'update-bar'}>
          {updateStatus.kind === 'available' ? (
            <>
              <p>有新版本 {updateStatus.version}，下载后会自动安装并重启。</p>
              <button type="button" className="compact" onClick={() => void applyUpdate()}>
                立即更新
              </button>
            </>
          ) : null}
          {updateStatus.kind === 'downloading' ? (
            <p>
              正在下载 {updateStatus.version}… {updateStatus.percent}%
            </p>
          ) : null}
          {updateStatus.kind === 'installing' ? <p>正在安装并重启…</p> : null}
          {updateStatus.kind === 'error' ? <p>{updateStatus.message}</p> : null}
        </div>
      ) : null}
      <header className="topbar">
        ...
        <div className="top-account">
          ...
          <span className="top-status">
            {loggingIn ? phaseLabel(login?.phase) : kindLabel(kind)}
          </span>
        </div>
      </header>
      ...
      <div className="toolbar">
        ...
        <div className="actions">
          {missingCli ? (
            ...
            <button type="button" className="secondary" disabled={busy || pullingId !== null} onClick={() => void startLogin()}>
          ...
          <div className="pane-head">
            ...
            <div className="pane-actions">
              ...
              <button
                ...
                onClick={() =>
                  selected ? void pullComments(selected.id) : void pullAllComments()
                }
              >
                {pullingId
                  ? '拉取中…'
                      ...
                      <div>
                        ...
                        <dd>
                          <button
                            type="button"
                            className="note-link"
                            onClick={() => setSelectedId(comment.noteId)}
                          >
...
function pullTone(result: XhsNotePullResult | null): string {
  ...
  if (result.message.startsWith('已写入') || result.message.startsWith('正在拉')) {
    return 'note-ok';
  }
  return 'note-error';
}
